/************************************************************
 * main.js – Dansk Læringsudgave, med 6 lokationer,
 * uden “Mere Info”-knapper under valg A/B,
...
// HTML-referencer (scoreboard, modaler, etc.)
...
let tutorialSteps = [
  {
    ...
    text: "Du forvalter LIMS på et stort hospital. Du styrer tid og penge, og hver beslutning har konsekvenser."
  },
  {
  ...
  },
  {
    ...
    text: `Dine mål: opnå mindst ${missionGoals.security} i sikkerhed og ${missionGoals.development} i udvikling.`
  }
];
let tutorialIdx = 0;
...
function showScenarioModal(stepIndex){
  ...
  if(t.digDeeperLinks && t.digDeeperLinks.length){
    ...
    t.digDeeperLinks.forEach(linkObj=>{
      ...
      // Viser en pop-up med linkObj.text
...
function applyChoiceEffect(eff){
  ...
  // synergyEffect, etc. – hvis relevant
}
