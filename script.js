...
const detailedScenarios = {
  "hospital": [
    ...
    // Flere scenarier for "hospital" kan du tilføje her...
  ],
  "it-jura": [
    {
    ...
    // Flere scenarier for "it-jura" kan du tilføje her...
  ],
  "leverandor": [
    {
    ...
    // Flere scenarier for "leverandor" kan du tilføje her...
  ],
  "infrastruktur": [
    {
    ...
    // Flere scenarier for "infrastruktur" kan du tilføje her...
  ],
  "informationssikkerhed": [
    ...
    // Flere scenarier for "informationssikkerhed" kan du tilføje her...
  ],
  "dokumentation": [
    {
...
function renderTasks() {
  ...
  gameState.availableTasks.forEach(t => {
    ...
    li.addEventListener("click", () => {
      li.querySelectorAll(".task-description").forEach(d => { d.style.display = (d.style.display === "none" ? "block" : "none"); });
    });
    li.appendChild(commitBtn);
    tasksList.appendChild(li);
  });
...
/* Når en lokation klikkes */
function handleLocationClick(locName) {
  // Vi bruger nu elementets id, så sørg for at sende el.id!
  if (!gameState.activeTask) { showPopup("Vælg en opgave først!", "error"); return; }
  if (gameState.time <= 0) return;
  const idx = gameState.activeTask.currentStep;
  if (idx >= gameState.activeTask.steps.length) return;
...
/* Tildel event listeners til lokationer – OBS! Nu bruger vi elementets id for konsistens */
...
// Ændr her: I stedet for at bruge nøglen fra objektet (som kan indeholde mellemrum), bruger vi hvert elements id.
