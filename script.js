 ...
 * - Sikring af at opgaver aldrig starter med "dokumentation" men altid slutter med det
 * - Ensartet stavning: Vi bruger nu "leverandor" (uden ø) for at matche index.html
 ************************************************************/
...
/* Tilladte lokationer – OBS! Vi bruger nu "leverandor" (uden ø) for at matche index.html */
...
const detailedScenarios = {
  "hospital": [
    ...
    // … (flere scenarier til "hospital" kan tilføjes her)
  ],
  "it-jura": [
    {
  ...
  "leverandor": [
    ...
    // … (flere scenarier for "leverandor")
  ],
  "infrastruktur": [
    {
    ...
    // … (flere scenarier for "infrastruktur")
  ],
  "informationssikkerhed": [
    ...
    // … (flere scenarier for "informationssikkerhed")
  ],
  "dokumentation": [
    {
...
function handleLocationClick(locName) {
  ...
  // Hvis næste trin er "dokumentation", skal dette håndteres automatisk
...
function finalizeCABResult() {
  ...
  if (Math.random() < gameState.finalFailChance) { showCABResult(false); }
...
function generateTask() {
  ...
  // Fjern "dokumentation" fra udvælgelsen
  const nonDocAllowed = allowed.filter(loc => loc.toLowerCase() !== "dokumentation");
  
  // Vælg antal besøg ud fra en vægtet fordeling – kun fra nonDocAllowed (typisk 5-6)
...
function assignTask(taskId) {
  ...
  } else { finalizeAssign(taskId, idx); }
}
...
function generateTask() {
  ...
  // Hent de tilladte lokationer for denne opgave (alle id'er er små)
  const allowed = allowedLocationsForTask[category];
  
  // Fjern "dokumentation" fra udvælgelsen, da den altid skal komme sidst
  const nonDocAllowed = allowed.filter(loc => loc.toLowerCase() !== "dokumentation");
  ...
  const newTask = {
    ...
    headline: taskName,
    description: getTaskDescription(category),
    steps: steps, // Unikke lokationer uden "dokumentation" midtvejs + "dokumentation" som sidste
    currentStep: 0,
    riskLevel: riskLevel,
    baseReward: baseReward,
    isHighPriority: (riskLevel === 3),
...
function finalizeStep() {
  ...
  gameState.activeTask.currentStep++;
  if (gameState.time <= 0) { endGame(); return; }
  if (gameState.activeTask.currentStep >= gameState.activeTask.steps.length) { showCABModal(); }
  else { updateStepsList(); }
}
...
function initGame() {
  ...
  setInterval(() => { if (gameState.time > 0 && gameState.availableTasks.length < 10) { generateTask(); } }, 10000);
}

/* Tildel event listeners til lokationer – brug de navne fra index.html */
...
Object.entries(locationElements).forEach(([locName, el]) => {
  el.addEventListener('click', () => { handleLocationClick(locName); });
});
