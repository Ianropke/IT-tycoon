 ...
 * - Ensartet stavning for alle lokationer (fx "leverandør")
 ************************************************************/
...
/* Tilladte lokationer – bemærk den rettede stavemåde "leverandør" */
...
const detailedScenarios = {
  "hospital": [
    ...
    // (Yderligere 9 scenarier – eksempler fra dine tabeller)
    {
      description: "Immunologiske analyser er forældede.",
      A: { label: "Konservativ Udvidelse", text: "2 tid, 50 kr; +1 stabilitet, +1 hospitalstilfredshed.", time: 2, money: 50, effects: { stability: 1, hospitalSatisfaction: 1 }, failBonus: 0 },
      B: { label: "Stor Modernisering", text: "5 tid, 150 kr; +3 hospitalstilfredshed, +2 udvikling, 5% fejlrisiko.", time: 5, money: 150, effects: { hospitalSatisfaction: 3, development: 2 }, failBonus: 0.05 }
    }
    // … (flere scenarier for "hospital")
  ],
  "it-jura": [
    {
  ...
  "leverandør": [
    ...
    // … (flere scenarier for "leverandør")
  ],
  "infrastruktur": [
    {
...
function generateTask() {
  ...
  // Fjern "dokumentation" fra udvælgelsen, så den ikke udvælges midtvejs
  const nonDocAllowed = allowed.filter(loc => loc.toLowerCase() !== "dokumentation");
  
  // Vælg antal besøg – med en fordeling der favoriserer 5-6, men kun fra nonDocAllowed
...
function generateTask() {
  ...
  // Hent de tilladte lokationer for opgavetypen
  const allowed = allowedLocationsForTask[category];
  
  // Fjern "dokumentation" fra udvælgelsen
  const nonDocAllowed = allowed.filter(loc => loc.toLowerCase() !== "dokumentation");
  
  // Vælg antal besøg ud fra en vægtet fordeling – kun fra nonDocAllowed
  const choices = [3, 4, 5, 6, 7];
  const weights = [0.1, 0.1, 0.4, 0.3, 0.1];
  let r = Math.random(), total = 0, numSteps = 3;
  for (let i = 0; i < choices.length; i++) {
...
function initGame() {
  ...
  setInterval(() => {
    if (gameState.time > 0 && gameState.availableTasks.length < 10) { generateTask(); }
  }, 10000);
}
